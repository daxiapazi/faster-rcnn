...
#字典按  dict2就可以按照方式dict2.num1进行访问
__C = edict()
...
def _merge_a_into_b(a, b):
  ...
  for k, v in a.items():
    ...
    if old_type is not type(v):
      if isinstance(b[k], np.ndarray):      #如果a,b中元素类型不一样 将a转换为b
...
def cfg_from_file(filename):
  ...
  import yaml                                  #yaml Yet Another Markup Language  配置文件
...
def cfg_from_list(cfg_list):
  ...
  for k, v in zip(cfg_list[0::2], cfg_list[1::2]):  #zip合并成元组
    key_list = k.split('.')
    d = __C
    ...
    assert subkey in d
    try:
      value = literal_eval(v)      #先检查下表达式的合法性
    except:
